...
cv::Ptr<cv::cuda::DensePyrLKOpticalFlow> alg_denseLK;
...
int main(int argc, char** argv){
  ...
  switch( type ){
    ...
    case 1:
      ...
      break;
    case 2:
      alg_denseLK = cv::cuda::DensePyrLKOpticalFlow::create();
    case 3:
  ...
  if( offset > v.length() && v.flag == false){
...
void ComputeFlow( const cv::Mat prev, const cv::Mat cur, const int type, const int bound, cv::Mat & flow_x, cv::Mat & flow_y ){
  ...
  case 1:
    ...
    break;
  case 2:
    alg_denseLK->calc(frame_0, frame_1, d_flow);
  case 3:
